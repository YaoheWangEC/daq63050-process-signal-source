...
/**
 * @brief  USART4 DMA RX IDLE ISR 封装
 *
 * 在 USART4 IDLE 中断中调用。负责暂停 DMA RX, 计算接收长度,
 * 调用 rx_handler 存入缓冲区并提取命令, 最后重配 DMA 并重启。
 */
void command_io_uart_rx_isr(void);

/**
 * @brief  UART DMA TX 完成 ISR 封装
 *
 * 在 DMA CH4 传输完成中断中调用。等待 USART TDC 标志, 切换回接收,
 * 解除发送忙状态。
 */
void command_io_uart_tx_isr(void);
